...
function Jogo() {
  ...
  return (
    <div>
      ...
      <br></br>
      {stop == false ? 
      <button onClick={() => { setAcao(true); setTime(Math.random() * 100); setCount(0); setStop(true) }}>Reiniciar</button> 
      :
       count >= 1 ? 
       <button onClick={() => setStop(false)}>Pare!!!!!</button> 
       :
        <button>Espere...</button>}
    </div>
  );
}
